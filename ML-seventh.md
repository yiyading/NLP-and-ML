...
**查准率Precision**和**查全率Recall**是新的度量标准。
...
* Precision = TP / (TP+FP)：对所有预测是恶性肿瘤的病人，实际上是恶性肿瘤的病人的比例。越高越好。

* Recall = TP / (TP+FN)：在所有恶性肿瘤病人中，成功预测有恶性肿瘤的病人的比例。越高越好。
...
**有别于逻辑回归输出的是概率。在SVM中，我们最小化后的代价函数h获得的参数θ，SVM用它直接预测y=1或者y=0**。因此当θ.T\*x大于等于0时，会直接预测y=1。这就是SVM数学上的意义。
